...
/* Tableau de comparaison entre encadrants. Les colonnes sont des taux,
   la dernière ligne est le restaurant : c'est elle qui donne l'échelle. */
.comparatif{width:100%;border-collapse:collapse;font-size:13px}
.comparatif th{text-align:right;padding:9px 8px;border-bottom:1px solid var(--line);
               font-size:10.5px;text-transform:uppercase;letter-spacing:.5px;
               color:var(--ink-3);font-weight:700}
.comparatif th:first-child{text-align:left}
.comparatif td{text-align:right;padding:10px 8px;border-bottom:1px solid var(--line);
               font-variant-numeric:tabular-nums}
.comparatif td:first-child{text-align:left;font-weight:650}
.comparatif tr.total td{background:var(--paper);font-weight:750;border-bottom:0}
.comparatif .haut{color:var(--vacances);font-weight:750}
.comparatif .tres-haut{color:var(--close);font-weight:750}
.comparatif .faible{color:var(--ink-3)}
...
async function vueEncadrants() {
  const [d, r] = await Promise.all([
    api("audit", "encadrants", { restaurant_id: E.resto }),
    api("audit", "ratios", { restaurant_id: E.resto }).catch(() => null)
  ]);
  ...
  $("#vue").innerHTML =
    ...
    '<p class="sous">Qui valide les comptages de caisse, sur toute la période chargée. ' +
      'Le <b>taux</b> est le nombre de comptages en manque de plus de 20 € divisé par le ' +
      'nombre de caisses validées. C\'est le seul chiffre comparable entre deux personnes, ' +
      'le montant cumulé suivant le volume de caisses.<br>' +
      'Un taux calculé sur moins de quatre comptages en manque n\'est pas comparé au ' +
      'restaurant : à ce niveau, un écart de plus ou de moins le fait doubler.</p>' +
    '<div class="compteurs">' +
    ...
    (l.length ? '<div class="bloc">' +
      ...
      l.map(function (e) {
        ...
        /* Un taux calculé sur deux comptages en manque ne se compare à
           rien : un événement de plus ou de moins le double. On exige un
           minimum de caisses validées ET de comptages en manque avant de
           situer quelqu'un par rapport au restaurant — sans quoi on
           désigne le hasard. */
        const comparable = e.validees >= 40 && e.en_manque >= 4;
        const ecart = !comparable
          ? " · trop peu d'écarts pour comparer"
          : (t !== null && ref !== null && ref > 0)
        ...
        return '<button class="ligne" onclick="vuePersonne(\'' + e.code + '\')">' +
          ...
          '<div style="text-align:right"><div class="montant num' +
            (t && comparable ? ' negatif' : '') + '" style="' +
            (comparable ? '' : 'color:var(--ink-3)') + '">' +
            (t === null ? "—" : t + " %") + '</div>' +
...
/* Quatre ratios de comportement, côte à côte. Aucun ne se lit dans
   l'absolu : c'est la dernière ligne, celle du restaurant, qui donne
   l'échelle. Un taux d'annulation de 1,2 % n'est remarquable que si les
   collègues sont à 0,3 %. */
const COLONNES_RATIOS = [
  ["taux_especes",   "Espèces",     "part du CA encaissée en liquide"],
  ["taux_remise",    "Remises",     "remises en % du CA déclaré"],
  ["taux_annulation","Annulations", "annulations en % du CA déclaré"],
  ["taux_correction","Corrections", "corrections de ticket en % du CA déclaré"]
];

function tableauRatios(r) {
  if (!r || !r.responsables || !r.responsables.length)
    return '<div class="bloc"><div class="carte-titre">Ratios par encadrant</div>' +
      '<div class="vide"><b>Pas encore de données</b>Déposez le rapport « Flux financiers ' +
      'cumulés par responsable de comptage » pour comparer les encadrants entre eux.</div></div>';

  const ref = r.restaurant;
  const cellule = function (e, champ) {
    const v = e[champ], m = ref[champ];
    if (v === null || v === undefined) return '<td class="faible">—</td>';
    // sans volume, un ratio ne se compare pas : on l'affiche en gris
    if (e.caisses < 20) return '<td class="faible">' + v + ' %</td>';
    let c = "";
    if (m && m > 0.01) {
      if (v >= m * 2) c = "tres-haut";
      else if (v >= m * 1.4) c = "haut";
    }
    return '<td class="' + c + '">' + v + ' %</td>';
  };

  return '<div class="bloc">' +
    '<div class="carte-titre">Ratios par encadrant</div>' +
    '<div class="note">Ces quatre taux décrivent des comportements de caisse, pas des ' +
      'écarts d\'argent. Ils se lisent par rapport à la ligne du restaurant, en bas : ' +
      'un taux élevé partout est une habitude du site, un taux élevé chez une seule ' +
      'personne est une question à poser. En ambre au-delà de 1,4 fois la référence, ' +
      'en rouge au-delà du double. Les encadrants sous 20 caisses restent en gris : ' +
      'leur ratio n\'est pas comparable.</div>' +
    '<div class="zone" style="overflow-x:auto">' +
    '<table class="comparatif"><thead><tr><th>Encadrant</th><th>Caisses</th>' +
      COLONNES_RATIOS.map(c => '<th title="' + esc(c[2]) + '">' + c[1] + '</th>').join("") +
    '</tr></thead><tbody>' +
    r.responsables.map(function (e) {
      return '<tr><td>' + esc(nom(e.code) !== e.code ? nom(e.code) : e.nom) + '</td>' +
        '<td class="faible">' + e.caisses + '</td>' +
        COLONNES_RATIOS.map(c => cellule(e, c[0])).join("") + '</tr>';
    }).join("") +
    '<tr class="total"><td>Restaurant</td><td>' + ref.caisses + '</td>' +
      COLONNES_RATIOS.map(c => '<td>' +
        (ref[c[0]] === null ? "—" : ref[c[0]] + " %") + '</td>').join("") + '</tr>' +
    '</tbody></table></div></div>';
}
...
async function vueAdmin() {
  ...
  E.utilisateurs = d.utilisateurs;
  E.perimetres = d.perimetres;
  $("#vue").innerHTML =
    '<h2>Accès</h2>' +
    ...
    '<div class="bloc"><div class="carte-titre">Personnes · ' + d.utilisateurs.length + '</div>' +
    d.utilisateurs.map(function(u){
      const n = d.perimetres.filter(p => p.utilisateur_id === u.id).length;
      return '<button class="ligne" onclick="ficheUtilisateur(' + u.id + ')">' +
        '<div class="corps"><div class="titre">' + esc(u.nom) + '</div>' +
        '<div class="detail">' + esc(u.role) + ' · ' + n + ' restaurant(s) · ' +
        (u.derniere_connexion ? "vu le " + jour(u.derniere_connexion) : "jamais connecté") +
        '</div></div>' + (u.actif ? "" : '<span class="etiquette">inactif</span>') +
        '<span class="resto-fleche">&#8250;</span></button>';
    }).join("") + '</div>' +
    '<div id="form-utilisateur"></div>' +
    '<div class="bloc"><div class="zone">' +
...
/* Un accès se modifie : les rôles changent, les périmètres bougent, un
   code s'oublie. Sans cet écran il fallait supprimer et recréer, ce qui
   fait perdre l'historique de connexion. */
function ficheUtilisateur(id) {
  const u = E.utilisateurs.find(x => x.id === id);
  if (!u) return;
  const p = E.perimetres.filter(x => x.utilisateur_id === id);
  const droit = (rid, champ) => {
    const l = p.find(x => x.restaurant_id === rid);
    return l && l[champ];
  };
  const ROLES = [["DIRECTEUR","Directeur"],["SUPERVISEUR","Superviseur"],
                 ["CDG","Contrôle de gestion"],["DG","Direction générale"]];

  document.getElementById("form-utilisateur").innerHTML =
    '<div class="bloc"><div class="zone">' +
      '<h3 style="margin:0 0 12px;font-size:15px">' + esc(u.nom) + '</h3>' +
      '<label class="champ"><span>Nom</span><input id="u-nom" value="' +
        esc(u.nom || "") + '"></label>' +
      '<label class="champ"><span>E-mail</span><input id="u-email" type="email" value="' +
        esc(u.email || "") + '"></label>' +
      '<label class="champ"><span>Rôle</span><select id="u-role">' +
        ROLES.map(r => '<option value="' + r[0] + '"' + (u.role === r[0] ? " selected" : "") +
          '>' + r[1] + '</option>').join("") + '</select></label>' +
      '<label class="champ"><span>Statut</span><select id="u-actif">' +
        '<option value="1"' + (u.actif ? " selected" : "") + '>actif</option>' +
        '<option value="0"' + (u.actif ? "" : " selected") + '>désactivé</option>' +
        '</select></label>' +
      '<label class="champ"><span>Nouveau code (laisser vide pour ne pas le changer)</span>' +
        '<input id="u-pin" inputmode="numeric" placeholder="••••••"></label>' +

      '<span class="champ"><span>Restaurants</span></span>' +
      '<div style="overflow-x:auto;margin-bottom:14px">' +
      '<table class="comparatif"><thead><tr><th>Restaurant</th><th>Consulter</th>' +
        '<th>Déposer</th><th>Clore</th></tr></thead><tbody>' +
      E.restos_admin.filter(r => r.actif !== false).map(function (r) {
        const c = (champ, suffixe) =>
          '<td><input type="checkbox" id="u-' + suffixe + '-' + r.id + '"' +
          (droit(r.id, champ) ? " checked" : "") + ' style="width:19px;height:19px;' +
          'accent-color:var(--navy)"></td>';
        return '<tr><td>' + esc(r.nom) + '</td>' +
          c("peut_lire", "lire") + c("peut_deposer", "dep") + c("peut_cloturer", "clo") +
          '</tr>';
      }).join("") + '</tbody></table></div>' +
      '<p class="sous" style="font-size:12.5px;margin-bottom:14px">Un directeur consulte et ' +
        'dépose son restaurant sans pouvoir clore un écart. Clore est réservé à la direction ' +
        'et aux superviseurs.</p>' +

      '<div id="err-utilisateur"></div>' +
      '<div class="actions" style="margin-top:0">' +
        '<button class="fort" onclick="enregistrerUtilisateur(' + id + ')">Enregistrer</button>' +
        '<button onclick="document.getElementById(\'form-utilisateur\').innerHTML=\'\'">' +
          'Annuler</button></div>' +
    '</div></div>';
  document.getElementById("form-utilisateur")
    .scrollIntoView({ behavior: "smooth", block: "nearest" });
}

async function enregistrerUtilisateur(id) {
  const perimetres = E.restos_admin.filter(r => r.actif !== false).map(function (r) {
    const lu = s => { const e = document.getElementById("u-" + s + "-" + r.id);
                      return !!(e && e.checked); };
    return { restaurant_id: r.id, peut_lire: lu("lire"),
             peut_deposer: lu("dep"), peut_cloturer: lu("clo") };
  }).filter(p => p.peut_lire || p.peut_deposer || p.peut_cloturer);

  try {
    const r = await api("audit", "majUtilisateur", { id: id,
      nom: $("#u-nom").value.trim(), email: $("#u-email").value.trim(),
      role: $("#u-role").value, actif: $("#u-actif").value === "1",
      pin: $("#u-pin").value.trim() || undefined, perimetres: perimetres });
    if (r && r.erreur) throw new Error(r.erreur);
    aller("admin");
  } catch (e) {
    $("#err-utilisateur").innerHTML = '<div class="alerte erreur">' + esc(e.message) + '</div>';
  }
}
